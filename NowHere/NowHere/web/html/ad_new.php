<?php
		...
		$result = $conn->query("SELECT Mem_Name, Contact_Phone FROM Member INNER JOIN Advertiser ON Member.Mem_No = Advertiser.Mem_No WHERE Member.Mem_No = '$userid';");
		$row = $result->fetch_assoc();
	
		$def_username = $row["Mem_Name"];
		$def_contact = $row["Contact_Phone"];
...
?>
								...
								<div class="form-group  row">
									...
									<input type="text" name="Start_Date" class="form-control" readonly><span class="input-group-addon"><i class="fa fa-calendar"></i></span></div>
									...
									<input type="text" name="End_Date" class="form-control"  readonly><span class="input-group-addon"><i class="fa fa-calendar"></i></span></div>
									<input type="hidden" id="dtp_input2" value="" /><br/>
								
								</div>
								<div class="form-group  row"><label class="col-sm-2 col-form-label">담당자 이름</label>
                                    <div class="col-md-4"><input type="text" class="form-control" name="Reg_Name" value="<?=$def_username?>"></div>
									<label class="col-sm-2 col-form-label" ">담당자 전화번호</label>
                                    <div class="col-md-4"><input type="text" class="form-control" name="Event_Contact_No" value="<?=$def_contact?>"></div>
								</div>
...
//                });
	...
	if (navigator.geolocation) {
		...
		navigator.geolocation.getCurrentPosition(function(position) {
			...
			document.getElementById('Lat').value = locPosition.getLat();
			document.getElementById('Lng').value = locPosition.getLng();
